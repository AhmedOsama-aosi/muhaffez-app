...
class AddtionItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    int totalWords, wrongwords, prograss = 0;
    int lastTringIndex = rciteItem.tringList.length - 1;
    if (lastTringIndex > -1) {
      totalWords =
          rciteItem.tringList[rciteItem.tringList.length - 1].totalWords;
      wrongwords = rciteItem
          .tringList[rciteItem.tringList.length - 1].wrongWordsList.length;
      prograss = (((totalWords - wrongwords) / totalWords) * 100).toInt();
    }
